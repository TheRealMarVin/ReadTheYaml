...
def test_factory_rejects_mixed_case_noNE():
    """Test that a mixed case like 'noNE' is rejected by the factory."""
...
def test_none_field_accepts_string_default():
    ...
    field = FIELD_FACTORY.create_field("NONE", name="my_field", description="test field", required=False, default="None")
    assert field.default == None

def test_none_field_rejects_bool_default():
    """Test that a boolean default value raises FormatError."""
    with pytest.raises(FormatError, match="invalid default value"):
        FIELD_FACTORY.create_field("NONE", name="my_field", description="test field", required=False, default=True)

def test_none_field_rejects_zero_default():
    """Test that integer zero default value raises FormatError."""
    with pytest.raises(FormatError, match="invalid default value"):
        FIELD_FACTORY.create_field("NONE", name="my_field", description="test field", required=False, default=0)

def test_none_field_rejects_string_default():
    """Test that an arbitrary string default value raises FormatError."""
    with pytest.raises(FormatError, match="invalid default value"):
        FIELD_FACTORY.create_field("NONE", name="my_field", description="test field", required=False, default="test")